...
> Describe the null hypotheses to which the _p_-values given in Table 3.4
> correspond. Explain what conclusions you can draw based on these _p_-values.
...
Yes. The _p_-value for the t-statistic for horsepower is extremely low, which suggests a relationship.
...
Yes, as the _p_-value calculated from the F-test is very low.
...
```{r}
lm.fita <- lm(Sales ~ Price + Urban + US, data = Carseats)
summary(lm.fita)
```
...
`Price`: For every $1 of extra cost, the number of sales decreases by around 50 units.

`Urban`: The _p_-value given by the t-test is very high, which means `Urban` is highly unlikely to be statistically significant to `Sales`. The coefficient tells us that in urban regions, the sales decrease by around 20 units. However, because of the high _p_-value, we should not draw conclusions about any relationship between `Urban` and `Sales` as we do not have enough evidence.

`US`: In US regions, the company's car seats tend to sell around 200 more units.

> c. Write out the model in equation form, being careful to handle the
>    qualitative variables properly.
>

$$Sales = 13 - 0.05 \cdot Price + \begin{cases}
1.18, \ \ \ \text{for US} = \text{Yes,  Urban} = \text{Yes}\\
1.20, \ \ \ \text{for US} = \text{Yes,  Urban} = \text{No}\\
-0.02, \text{for US} = \text{Yes,  Urban} = \text{Yes}\\
0,\ \ \ \ \ \ \ \ \text{for US} = \text{No,  Urban} = \text{No}
\end{cases}$$
...
`Price` and `US`, as their _p_-values from the t-test are sufficiently low.
...
Not very well - both have an $R^2$ statistic of 0.2393, which means only 24% of the variance of the data can be explained by either model. (e)'s model has a slightly smaller RSE at 2.469 compared to (a)'s 2.472, but both do not fit the data very well.

```{r}
anova(lm.fite, lm.fita)
```
The ANOVA test shows that part (a) fits (insignificantly) better.
...
```{r}
plot(predict(lm.fite), rstudent(lm.fite), xlab = "Fitted values", ylab = "Studentised residuals", pch=20)
```
No values sit outside the -3 to 3 range for studentised residuals, so none seem to be outliers.

```{r}
plot(hatvalues(lm.fite), rstudent(lm.fite), xlab = "Leverage", ylab = "Studentised residuals", pch=20)
```

There is one significantly high leverage observation in the model.
...
summary(lm.fit)
```
`x` is highly correlated with `y` (as is to be expected, since `y` is a linear transformation on `x`)
